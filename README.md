...
1. **页面便捷新建与管理**：
   ...
   - **支持拖拽排序**：鼠标按住各卡片左侧或空白区域即可上下拖拽自由调整排序，修改保存后顺序保持稳定。
   - **顶栏测试通知**：卡片顶栏提供便捷的【🔔 测试通知】按钮，一键向企业微信发送全量耗材总览报告进行连通性测试。

2. **直观现代的进度与历史对比**：
   - 显示开始与预设结束的年月（如 `2025/11 ~ 2026/11`）。
   - 极简平滑的色彩动态胶囊进度条：采用绿到黄平滑渐变，居中清晰内嵌显示 `剩余 XX 天 (XX%)` 与起始/结束年月。
   - **历史弹窗对比**：卡片顶栏右侧提供独立的【历史】弹窗按钮（位于【已更换】左侧），点击即可唤起历次更换周期对比列表，不占用卡片常驻高度，界面更纯粹紧凑。
